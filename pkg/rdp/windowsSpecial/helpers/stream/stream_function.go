...
func (streamer *WindowsStreamHelper) buildGstVideoPipeline(config *rdp.StreamConfig) string {
	...
	case "intel":
		...
		log.Printf("DEBUG: GST-PIPELINE: %s", pipeline)
...
func (streamer *WindowsStreamHelper) StartStream(config *rdp.StreamConfig, pipelineFactory func(*rdp.StreamConfig) string) error {
	...
	if err := pipeline.SetState(gst.StatePlaying); err != nil {
		log.Printf("ERROR: Could not start stream for %s, err: %v", pipelineString, err)
	}
	log.Printf("DEBUG: Readied pipeline %s", pipelineString)
	return nil
}

func (streamer *WindowsStreamHelper) Close() error {
	if streamer.pipelines == nil {
		return nil
	}
	log.Printf("DEBUG: Closing GST Streams\n")
	...
	for _, p := range streamer.pipelines {
		if err := p.SetState(gst.StateNull); err != nil {
			log.Printf("ERROR: COULD NOT STOP PIPELINE FROM PLAYING STATE, %v! This does not stop the close process!", err)
		}
		// stop before unref
...
func (streamer *WindowsStreamHelper) Start(config *rdp.StreamConfig) error {
	gst.Init(nil)
	...
	if err := streamer.StartStream(config, streamer.buildGstVideoPipeline); err != nil {
		log.Printf("ERROR: Could not start video stream, %v", err)
		return err
	}

	// start audio
	if err := streamer.StartStream(config, streamer.buildGstAudioPipeline); err != nil {
		log.Printf("ERROR: Could not start audio stream %v", err)
		return err
	}
	go func() {
		...
		go func() {
			<-ctx.Done()
			log.Println("DEBUG: Context canceled: stopping main loop")
			streamer.mainLoop.Quit()
		}()

		// Run the GLib main loop
		log.Println("DEBUG: Starting GStreamer main loop")
		streamer.mainLoop.Run()
		log.Println("DEBUG: GStreamer main loop stopped")
	}()

	return nil
}

func (streamer *WindowsStreamHelper) MsgProcessor(data []byte) {
	// stub
}

func (streamer *WindowsStreamHelper) RegisterMsgSender(conn net.Conn) {
	streamer.conn = conn
