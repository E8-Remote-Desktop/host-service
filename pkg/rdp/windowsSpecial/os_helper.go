...
func (helper *WindowsOSHelper) RestartInteractiveServices(desktopName string, token windows.Token) {
...
func (helper *WindowsOSHelper) MonitorDesktops() {
	...
	for {
		...
		if currentUser != lastUser {
			...
			log.Printf("DEBUG: Switched to new token for execution")
			lastUser = currentUser
			log.Printf("DEBUG: Restarting Stream and Input due to Token Change")
			forceRestart = true
		}
		...
		if forceRestart {
			helper.RestartInteractiveServices(lastDesktop, token)
			forceRestart = false
		}
		time.Sleep(500 * time.Millisecond)
	}
